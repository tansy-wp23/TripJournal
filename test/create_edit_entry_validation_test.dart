...
void main() {
  testWidgets('new entry presents a clear section hierarchy', (tester) async {
  ...
  testWidgets(
    'create-entry form validation blocks a totally empty entry, but title-only is enough',
    (WidgetTester tester) async {
      ...
      await tester.pump();
      expect(
        find.text('Please add a title or write something in your entry.'),
        findsOneWidget,
      );
  ...
  testWidgets('a body alone (no title) is also enough to save', (
    WidgetTester tester,
  ) async {
    tester.view.physicalSize = const Size(1200, 2600);
    tester.view.devicePixelRatio = 1.0;
