...
void main() {
  ...
  group('AiRequestMonitoringScreen', () {
    testWidgets('loads and shows every seeded entry', (tester) async {
      ...
      expect(find.text('View failed requests'), findsOneWidget);
      expect(tester.widget<AppBar>(find.byType(AppBar)).actions, isEmpty);

      expect(
        find.byKey(const Key('admin-ai-requests-results')),
        findsOneWidget,
      );
    ...
    testWidgets('tapping the Failed status chip narrows to failed entries', (
      tester,
    ) async {
      await seedTwoEntries();
      await pumpScreen(tester, AiRequestMonitoringController(repository));
      ...
      await tester.tap(
        find.byKey(const Key('admin-ai-requests-status-failed')),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('tapping All statuses after a filter restores the full list', (
      tester,
    ) async {
      await seedTwoEntries();
      await pumpScreen(tester, AiRequestMonitoringController(repository));
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.byKey(const Key('admin-ai-requests-clear-filters')),
          findsNothing,
        );
    ...
    testWidgets(
      'the failed-requests action navigates to FailedAiRequestsScreen',
      (tester) async {
        await pumpScreen(tester, AiRequestMonitoringController(repository));
    ...
    testWidgets('no entries at all shows the empty state, not a blank list', (
      ...
      expect(
        find.byKey(const Key('admin-ai-requests-empty-state')),
        findsOneWidget,
      );
    ...
    testWidgets('a failing repository shows an error with a retry button', (
      ...
      await pumpScreen(
        tester,
        AiRequestMonitoringController(_FailingAiRequestLogRepository()),
      );

      expect(
        find.byKey(const Key('admin-ai-requests-retry-load')),
        findsOneWidget,
      );
    });
  });
