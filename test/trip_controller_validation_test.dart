...
void main() {
  ...
  test('createTrip rejects a title over 100 chars — validation lives in the controller, not just the form', () async {
    final trip = newTrip(
      id: 'new-trip',
      title: 'a' * 101,
  ...
  test('createTrip rejects overlapping dates against the seeded active trip', () async {
  ...
  test('editTrip rejects end-before-start', () async {
    ...
    final invalid = osaka.copyWith(endDate: osaka.startDate.subtract(const Duration(days: 1)));
  ...
  test('editTrip does not collide with its own unchanged dates', () async {
    ...
    expect(error, isNull);
    expect(controller.trips.firstWhere((t) => t.id == 'trip-002').title, 'Osaka Trip (renamed)');
  });
