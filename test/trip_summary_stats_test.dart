...
void main() {
  group('computeTripStats', () {
    test('counts distinct logical entry dates instead of timestamp dates', () {
    ...
    test(
      'sums steps/calories eaten and counts distinct logged days, not raw entry count',
      () {
        final entries = [
          _entry(
            id: 'e1',
        ...
        expect(
          ...
          300,
        ); // only e1 had data — partial sum, not null
    ...
    test(
      'totalCaloriesBurned is null (not zero) when no entry has burned data',
      () {
        final entries = [
          _entry(
            id: 'e1',
        ...
        expect(
          stats.totalCaloriesEaten,
          500,
        ); // caloriesEaten is unaffected — always summed
      },
    );

    test(
      'empty entries produce zero totals, null burned calories, and a null average mood',
      () {
    ...
    test('a single mood rounds to itself', () {
      final stats = computeTripStats(
        entries: [
          _entry(id: 'e1', createdAt: DateTime(2026, 1, 1), mood: Mood.tired),
        ],
        totalDays: 1,
      );
  ...
  test(
    'tripStatsForTrip pulls entries via the journal repository for a real seeded trip',
    () async {
