...
void main() {
  testWidgets('shows "write today\'s entry" when no entry exists for today', (tester) async {
  ...
  // having happened yet.
  testWidgets('switches to the confirmation state once today\'s entry exists', (tester) async {
    tester.view.physicalSize = const Size(1200, 2600);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    addTearDown(tester.view.resetDevicePixelRatio);
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const Key('entry-title-field')), 'Nudge test entry');
    await tester.enterText(find.byKey(const Key('entry-body-field')), 'Body written for today.');
