...
void main() {
  testWidgets('active trip uses the aurora hero and exposes its destination', (
    tester,
  ) async {
    ...
    expect(find.byType(AuroraPanel), findsOneWidget);
    expect(
      find.bySemanticsLabel('Open active trip Kyoto Trip'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'tapping the stats area of the active-trip card opens the trip (no dead zone)',
    (tester) async {
      tester.view.physicalSize = const Size(1200, 2600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets(
    'tapping "Write today\'s entry" opens the entry form, not the trip view',
    (tester) async {
      tester.view.physicalSize = const Size(1200, 2600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
