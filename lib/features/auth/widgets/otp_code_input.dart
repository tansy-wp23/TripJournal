...
class _OtpCodeInputState extends State<OtpCodeInput> {
  ...
  void _onChanged(int index, String value) {
    ...
    // (The TextField can't receive the backspace key directly; instead we
    // detect it by an empty value arriving for an already-empty box.)
    if (value.isEmpty && _digits[index].isEmpty && index > 0) {
      _digits[index - 1] = '';
      ...
      children: List.generate(widget.length, (i) {
        return Container(
          ...
          child: TextField(
            ...
            // Pass the FocusNode directly to the TextField so requestFocus()
            // actually moves the cursor (fixes the auto-advance bug noted in
